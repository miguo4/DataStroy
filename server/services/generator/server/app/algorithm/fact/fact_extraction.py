...
def fact_extraction(fact, df):
    ...
    groupbys = list(map(lambda x:x['field'], fact['breakdown']))
    ...
    measure_df['COUNT'] = 1
    if len(measures) == 0:
        fact_df = measure_df.groupby(fact['breakdown'][0]['field'], as_index=False).agg({'COUNT':'count'})
    elif len(measures) == 1:
        if 'aggregate' in fact['measure'][0]:
            ...
            if aggregation == '':
                ...
                fact_df = measure_df.groupby(fact['breakdown'][0]['field'], as_index=False).agg({fact['measure'][0]['field']:aggregation, 'COUNT':'count'})
        else:
            fact_df = measure_df
    elif len(measures) == 2:
        if 'aggregate' in fact['measure'][0] and 'aggregate' in fact['measure'][1]:
            ...
            if aggregation1 == 'avg':
                aggregation1 = 'mean'
            fact_df = measure_df.groupby(fact['breakdown'][0]['field'], as_index=False).agg({fact['measure'][0]['field']:aggregation0, fact['measure'][1]['field']:aggregation1, 'COUNT':'count'})
        else:
            fact_df = measure_df
