...
def categorization(fact, fact_df, method):
    ...
    N = len(count_list)
    try:
        parameter = fact_df[fact['breakdown'][0]['field']].tolist() # category list
    except:
        return 0 , []
    if method == "nosig":
        return 1, parameter
    ...
    if N > 12 or fact_df[fact['breakdown'][0]['field']].dtype == 'datetime64[ns]':
        if fact_df[fact['breakdown'][0]['field']].dtype == 'datetime64[ns]':
